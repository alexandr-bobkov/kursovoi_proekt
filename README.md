...
> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №1: Паспорт инфраструктуры каталога]**
> *Что вставить:* Скриншот главной сводной панели (Dashboard) вашего каталога `kursovaya-enterprise-variant` в веб-консоли Яндекса. На нем должно быть наглядно видно общее количество созданных ресурсов: 5 ВМ, 5 дисков, 1 балансировщик, 1 HTTP-роутер, 7 подсетей и 2 SSL-сертификата.
...
> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №2: Облачные сети и DNS]**
> *Что вставить:* Скриншот из раздела «Virtual Private Cloud» -> «Облачные сети» -> вкладка «Внутренние зоны DNS». Должен быть виден список сервисных приватных зон `internal.` и `10.in-addr.arpa.`, привязанных к вашей сети.

---

## 3. Вычислительные ресурсы и автоматическое масштабирование (vms.tf)

Для развертывания компонентов распределенной системы использовано 5 виртуальных машин под управлением современной и стабильной операционной системы Debian 13 (Trixie). С целью жесткого соблюдения экономических ограничений бюджета выделенного учебного гранта, все виртуальные машины запущены в **прерывистом (Preemptible) режиме** с использованием экономичных сетевых жестких дисков (network-hdd).
...
4. **`web-servers-instance-group`** (Динамические приватные IP: `10.100.20.7` в зоне А и `10.100.30.25` в зоне Б): масштабируемая группа веб-серверов.
...
> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №3: Виртуальные машины]**
> *Что вставить:* Скриншот из раздела «Compute Cloud» -> «Виртуальные машины» в консоли Яндекса. Должен быть виден список всех 5 запущенных серверов с зелеными значками статуса `Running`, их приватными и публичными IP, а также маркером прерывистого режима работы.
...
* Для защиты пользовательских данных на ALB настроен TLS-слушатель (Listener) на порту **443 (HTTPS)**. Взаимодействие балансировщика с сервисом **Yandex Certificate Manager** позволяет использовать активный SSL-сертификат `site-ssl-certificate` для учебного домена `bobkov-devops.ru`.
...
> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №4: Работа балансировщика и сайта]**
> *Что вставить:* Открытую в браузере страницу вашего сайта по защищенному адресу `https://158.160.179`. На скриншоте должен быть четко виден «замочек» безопасного соединения HTTPS, ФИО студента («Бобков А.К.») и динамический блок с аппаратными данными активной веб-ноды (web-node-0 или web-node-1), обработавшей запрос в данный момент.
...
> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №5: Дашборд Grafana]**
> *Что вставить:* Окно браузера с открытой панелью Grafana по адресу `http://89.169.134.76:3000`. Должен быть открыт импортированный дашборд `Node Exporter Dashboard Enterprise` с живыми графиками загрузки CPU ваших серверов бэкенда.
...
* На Бастион-хосте запущен контейнер **Kibana** (порт 5601), предоставляющий администратору удобную веб-консоль для сквозного поиска логов всей сети в едином окне через инструмент `Discover`.

В качестве дополнительного уровня безопасности задействован нативный облачный сервис **Yandex Cloud Logging** (активная группа логов `default`), который независимо от виртуальных машин агрегирует аудит-события самой платформы (Control Plane Яндекса) со сроком ротации данных в 3 дня.

> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №6: Интерфейс Kibana Discover]**
> *Что вставить:* Окно браузера с открытой панелью Kibana по адресу `http://89.169.134.76:5601`. Должна быть открыта вкладка `Discover` с выбранным индекс-паттерном `nginx-logs-*`, отображающая гистограмму запросов и поток текстовых логов, прилетающих от Filebeat с серверов `web-node`.
...
В коде Terraform развернуто автоматическое расписание под названием `infrastructure-daily-backup-plan`, функционирующее по следующим жестким Enterprise-правилам:
* **Периодичность и время:** Создание полных снимков (слепков) дисков происходит в автоматическом режиме каждую ночь ровно в **02:00 UTC**. Данный временной промежуток выбран искусственно, так как он совпадает с периодом наименьшей активности пользователей на веб-сайте и минимизирует нагрузку на дисковую подсистему хранения.
...
> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №7: Расписание снимков дисков]**
> *Что показать на скриншоте:* Раздел «Compute Cloud» -> «Снимки дисков» -> вкладка «Расписания» в консоли управления Yandex Cloud. На снимке экрана должно быть четко видно созданное ежедневное расписание, глубина хранения в 7 дней и список из пяти привязанных к нему дисков ваших облачных машин.

---

## 8. Архитектурная схема проекта

Для демонстрации сетевой связности, используемых внутренних портов, направления потоков данных и протоколов взаимодействия между компонентами распределенной системы была разработана детальная архитектурная карта инфраструктуры.

> 📷 **[МЕСТО ДЛЯ СКРИНШОТА №8: Схема Draw.io]**
> *Что вставить на это место:* Экспортированное изображение вашей финальной просторной схемы из Draw.io, где динамическая группа инстансов объединена в общий зеленый блок `web_group`, а стрелки трафика (`HTTPS:443`/`HTTP:80`), мониторинга (`Pull:9100`), управления (`ProxyJump`/`Kibana`) и логирования (`Filebeat`) разнесены по разным коридорам и не пересекают текст кубиков серверов.

### 🧠 Сравнительный анализ архитектурных подходов: Вариант "Base" против Варианта "Enterprise"

Главное архитектурное отличие корпоративного (Enterprise) варианта от базового заключается в переходе от статичной структуры «одиночных серверов» к динамической, масштабируемой и полностью изолированной отказоустойчивой среде, готовой к промышленным нагрузкам.

| Критерий сравнения | Базовый вариант (Base) | Корпоративный вариант (Enterprise) |
| :--- | :--- | :--- |
| **1. Организация веб-серверов** | **Одиночные статичные ВМ.** В облаке разворачиваются два фиксированных независимых сервера. При резком наплыве пользователей процессоры перегружаются, вызывая отказ в обслуживании (DDoS-эффект). | **Динамическая группа инстансов.** Используется масштабируемый кластер `yandex_compute_instance_group` с политикой **Auto Scale**. При достижении нагрузки в 70% CPU облако само мгновенно разворачивает дополнительную веб-ноду. |
| **2. Безопасность и шифрование** | **Незашифрованный HTTP (порт 80).** Пользовательский трафик идет в открытом текстовом виде через интернет, что уязвимо для перехвата данных (атак Man-in-the-Middle). | **Шифрование HTTPS (порт 443).** Реализована схема **SSL Termination** на балансировщике ALB с интеграцией сервиса **Certificate Manager** и активным SSL-сертификатом для домена `bobkov-devops.ru`. |
| **3. Сетевая связность и Discovery** | **Статическая IP-адресация.** Компоненты системы общаются по жестко прописанным в файлах конфигурации внутренним адресам. | **Сервисный Cloud DNS.** Поскольку в группе инстансов серверы создаются и удаляются динамически, развернуты внутренние зоны DNS (`internal.`), обеспечивающие механизм **Service Discovery** (автоматическое обнаружение новых нод). |
| **4. Глубина сбора логов (EFK)** | **Только прикладные логи Nginx.** Агент собирает стандартные файлы посещаемости веб-сервера `access.log` и `error.log`. | **Сквозной аудит всей системы.** Дополнительно собираются критические журналы ОС: `syslog` (системные ошибки) и `auth.log` (аудит SSH-доступа), а также аудит-логи самой платформы через **Yandex Cloud Logging**. |
| **5. Механизм оркестрации** | **Прямой деплой Ansible.** Скрипты автоматизации подключаются к серверам напрямую по их известным статическим адресам. | **ProxyJump-туннелирование.** Ansible осуществляет оркестрацию изолированных приватных серверов строго через защищенный шлюз Бастион-хоста, исключая нахождение управляющих портов (SSH:22) во внешней сети. |

---

### 💡 Главные тезисы для защиты перед комиссией (Козыри на оценку «Отлично»):

Если на защите вас попросят подвести итог, почему ваш Enterprise-вариант лучше, озвучьте эти три ключевых преимущества:

1. **Эластичность и экономия ресурсов**: *«В отличие от варианта Base, где серверы простаивают и расходуют бюджет гранта в часы затишья, Enterprise-вариант с Auto Scaling гибко подстраивается под реальный трафик, разворачивая мощности только тогда, когда это действительно необходимо»*.
2. **Нулевая зона доверия (Zero Trust)**: *«Все вычислительные ресурсы, базы данных логов и мониторинга полностью скрыты в приватном контуре. Единственная точка входа для пользователей — интеллектуальный балансировщик с SSL-шифрованием, а для администратора — защищенный Бастион-хост. Прямая атака на бэкенд из интернета физически невозможна»*.
3. **Отказоустойчивость инфраструктуры**: *«Распределение динамической группы инстансов по разным дата-центрам Яндекса (зона А и зона Б) в сочетании с непрерывным HTTP Health Check на балансировщике гарантирует, что сайт продолжит работать без прерывания обслуживания, даже если один из дата-центров облака полностью выйдет из строя»*.
