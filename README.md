...
Мониторинг организован по модели сбора данных Pull. На всех виртуальных машинах запущен агент **Prometheus Node Exporter** (порт 9100). На веб-серверах развернут официальный нативный пакет `prometheus-nginx-exporter` (порт 9113), привязанный к рабочему эндпоинту Nginx `127.0.0.1:80/stub_status`.

Центральный сервер Prometheus опрашивает всех агентов по их приватным IP-адресам. Визуализация метрик выведена в **Grafana**, развернутую на выделенном хосте `grafana-host`. Настройка подключения Prometheus и импорт официальных дашбордов Node Exporter (`1860`) и Nginx (`12708`) происходят полностью автоматически через Grafana API без ручных действий.

С помощью блока `relabel_configs` в Prometheus настроено динамическое переименование хостов, благодаря чему на плашках и графиках отображается красивый и понятный формат имен: **`10.0.10.19 (web1)`** и **`10.0.20.34 (web2)`** вместо безликих IP-адресов. Все таргеты находятся в статусе **`UP`**.
...
* Снимки (слепки) дисков создаются автоматически каждую ночь в **00:00 UTC**, в период наименьшей активности.
